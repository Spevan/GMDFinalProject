	...
	public class ScriptableSheetsEditorWindow : EditorWindow, IHasCustomMenu
	{
		...
		private void OnEnable()
		{
			...
			if (windowSessionState == null)
			{
				...
				m_TableLayouts = new Dictionary<string, TableLayout>();
			}
			else
			{
		...
		private void OnGUI()
		{
			...
			{
				EditorGUILayout.EndHorizontal();
				var mainAssetPath = m_MainAsset != null ? '/' + m_MainAsset.name + UnityConstants.Extensions.Asset : string.Empty;
				EditorGUILayout.HelpBox($"Did not find any objects of type {m_SelectedType} under path {m_Settings.ObjectManagement.Scan.Path}{mainAssetPath}. Try updating the path or creating a new asset.", MessageType.Warning);
				m_Paginator.GoToFirstPage();
				m_SortedObjects.Clear();
				return;
			}
			...
			{
				var defaultNewAssetPath = AssetDatabase.GetAssetPath(filteredObjects[0]).Replace($"{filteredObjects[0].name}{UnityConstants.Extensions.Asset}", string.Empty);
				// Check for subasset paths.
				if (defaultNewAssetPath.EndsWith(UnityConstants.Extensions.Asset))
				{
